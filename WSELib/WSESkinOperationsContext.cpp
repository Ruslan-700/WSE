...
int FaceKeysGetMorphKey(WSESkinOperationsContext *context)
{
	...
	wb::face_keys face_keys;
	face_keys.from_string(str);
	return (int)(face_keys.get_morph_key(key_no) * 7.0f);
}

void FaceKeysSetMorphKey(WSESkinOperationsContext *context)
{
	int sreg, key_no, value;
	...
	wb::face_keys face_keys;
	face_keys.from_string(warband->basic_game.string_registers[sreg]);
	face_keys.set_morph_key(key_no, value / 7.0f);
	warband->basic_game.string_registers[sreg] = face_keys.to_string();
}

WSESkinOperationsContext::WSESkinOperationsContext() : WSEOperationContext("skin", 5200, 5299)
{
}
