...
namespace
{
	const unsigned __int64 MAX_MODULE_TEXT_FILE_SIZE = 256ULL * 1024ULL * 1024ULL;

	int pushModuleTextFileError(lua_State *L, const char *message, DWORD errorCode)
	{
		lua_pushnil(L);
	...
	bool isAsciiModuleFileNameCharacter(char c)
	{
		return (c >= 'a' && c <= 'z') ||
			(c >= 'A' && c <= 'Z') ||
			(c >= '0' && c <= '9') ||
	...
	bool isReservedWindowsFileName(const char *fileName)
	{
		const char *firstDot = strchr(fileName, '.');
		size_t stemLength = firstDot ? firstDot - fileName : strlen(fileName);

		if (stemLength == 3)
		{
			return _strnicmp(fileName, "CON", 3) == 0 ||
				_strnicmp(fileName, "PRN", 3) == 0 ||
				_strnicmp(fileName, "AUX", 3) == 0 ||
				_strnicmp(fileName, "NUL", 3) == 0;
		}

		if (stemLength == 4 && fileName[3] >= '1' && fileName[3] <= '9')
		{
			return _strnicmp(fileName, "COM", 3) == 0 ||
				_strnicmp(fileName, "LPT", 3) == 0;
		}

		return false;
	}

	bool isSafeModuleTextFileName(const char *fileName)
	{
		if (!fileName)
			return false;
		...
		{
			if (!isAsciiModuleFileNameCharacter(fileName[i]))
				return false;
		}

		if (_stricmp(fileName + length - 4, ".txt") != 0)
			return false;
...
REG(readModuleTextFile)
int lc_readModuleTextFile(lua_State *L)
{
	if (lua_gettop(L) != 1 || lua_type(L, 1) != LUA_TSTRING)
		return luaL_error(L, "readModuleTextFile expects one string argument");

	const char *fileName = lua_tostring(L, 1);
	if (!isSafeModuleTextFileName(fileName))
		return pushModuleTextFileError(L, "invalid module text file name", ERROR_INVALID_NAME);

	std::string modulePath = warband->cur_module_path.c_str();
	if (modulePath.empty())
		return pushModuleTextFileError(L, "module directory is unavailable", ERROR_PATH_NOT_FOUND);
	...
	if (fullPath.length() >= MAX_PATH)
		return pushModuleTextFileError(L, "module text file path is too long", ERROR_FILENAME_EXCED_RANGE);
	...
	if (file == INVALID_HANDLE_VALUE)
		return pushModuleTextFileError(L, "could not open module text file", GetLastError());
	...
	if (!GetFileInformationByHandle(file, &fileInfo))
	{
		...
		CloseHandle(file);
		return pushModuleTextFileError(L, "could not inspect module text file", errorCode);
	}
	...
	{
		CloseHandle(file);
		return pushModuleTextFileError(L, "module text file is not a regular file", ERROR_ACCESS_DENIED);
	}

	ULARGE_INTEGER fileSize;
	fileSize.HighPart = fileInfo.nFileSizeHigh;
	fileSize.LowPart = fileInfo.nFileSizeLow;
	if (fileSize.QuadPart > MAX_MODULE_TEXT_FILE_SIZE)
	{
		CloseHandle(file);
		return pushModuleTextFileError(L, "module text file is too large", ERROR_FILE_TOO_LARGE);
	}
	...
	{
		CloseHandle(file);
		return pushModuleTextFileError(L, "not enough memory to read module text file", ERROR_NOT_ENOUGH_MEMORY);
	}

	size_t totalBytesRead = 0;
	while (totalBytesRead < contents.size())
	{
		DWORD bytesToRead = (DWORD)std::min<size_t>(contents.size() - totalBytesRead, 1024 * 1024);
		DWORD bytesRead = 0;
		if (!ReadFile(file, &contents[totalBytesRead], bytesToRead, &bytesRead, NULL))
		{
			DWORD errorCode = GetLastError();
			CloseHandle(file);
			return pushModuleTextFileError(L, "could not read module text file", errorCode);
		}

		if (bytesRead == 0)
		{
			CloseHandle(file);
			return pushModuleTextFileError(L, "module text file ended unexpectedly", ERROR_HANDLE_EOF);
		}
	...
	CloseHandle(file);
	lua_pushlstring(L, contents.empty() ? "" : contents.data(), contents.size());
	return 1;
}
